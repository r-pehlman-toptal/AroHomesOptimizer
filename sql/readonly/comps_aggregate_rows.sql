-- Row-level comps with weights (same filters and scoring as comps_aggregate.sql).
-- OPTIMIZED: spatial-first (nearby_geom), then join mls_history; no full-table geom/addr scan.
-- Parameters: subject_parcel_id, subject_sqft, property_subtype, distance_cap_miles, size_band_pct, recency_months, ppsf_min, half_life_months, half_life_miles, city_name, limit.
